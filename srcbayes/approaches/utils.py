...
class BayesianAdam(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        super(BayesianAdam, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(BayesianAdam, self).__setstate__(state)
        for group in self.param_groups:
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if isinstance(group['lr'], torch.Tensor):
                    #p.data = p.data + torch.mul(-group['lr'].data, d_p)
                    p.add_(torch.mul(-step_size, torch.true_divide(exp_avg, denom)))
                else:
